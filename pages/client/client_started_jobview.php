...
if (!isset($_GET['jobid'])) {
    header("Location: client_started_jobs.php");
}
$joblist = $job_collection->getFreelancerStarted($_GET['jobid']);
$propos = $proposal->getAllJobProposal($_GET['jobid']);
$freelancer = $info->getImportantInfo($propos[0]['owner']);
...
<body>
                    ...
                    <?php if (empty($income->checkIfDone($_GET['jobid']))) {
                        if ($propos[0]['delivered'] == 1) { ?>
                            <div class="box-rcv">
                                ...
                                <h6>Already send the output </h6>
                                <button class="ord-btn btn-rcv" data-price="<?php echo $propos[0]['proposal_price']; ?>" data-proposalid="<?php echo $propos[0]['id']; ?>" data-jobid="<?php echo $_GET['jobid']; ?>" data-freelancer="<?php echo $propos[0]['owner']; ?>">Output Received</button>
                                <a href="../../assets/deliver/<?php echo $propos[0]['delivered_item']; ?>" class="">
                        ...
                        <?php } else { ?>
                            <div class="box-rcv">
                                ...
                                <h6>The freelancer already working on your jobs.</h6>
                            </div>
                        <?php } ?>
                    <?php } else { ?>
                        <div class="box-rcv">
                            <i class="fa-solid fa-thumbs-up"></i>
                            <h6>Job Already Done.</h6>
                            <br>
                            <a href="../../assets/deliver/<?php echo $propos[0]['delivered_item']; ?>" class="">
                                <button class="view-btn">View</button>
                            </a>
    ...
    <script>
        $(document).on('click', '.btn-rcv', function() {
            let proposalid = $(this).attr('data-proposalid');
            let jobid = $(this).attr('data-jobid');
            let freelancer = $(this).attr('data-freelancer');
            let price = $(this).attr('data-price');
            swal({
                    title: "Are you sure want to recieved?",
                    text: "Double check the output. This process can't be undo.",
                    icon: "info",
                    buttons: true,
                    dangerMode: false,
                })
